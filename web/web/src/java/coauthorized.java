...
@WebServlet(urlPatterns = {"/coauthorized"})
public class coauthorized extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{  
                ...
                ResultSet rs2=stmt2.executeQuery("select * from author");
                
                int zhangPaperId= 0;
                int luPaperId= 0;
                String roleZhang = null;
                String roleLu = null;
                
                while(rs2.next()){
                    if(rs2.getString(2).equals("Zhang")){
                        zhangPaperId = rs2.getInt(6);
                        roleZhang = rs2.getString(4);
                    }else if(rs2.getString(2).equals("Lu")){
                        luPaperId = rs2.getInt(6);
                        roleLu = rs2.getString(4);
                    }
                }
                ...
                "</style>"
                        ...
                        "  <br><br><h2>Conference Papers Coauthorized By Zhang And Lu</h2>"+
                        "  <p> </p>\n" +
                        "  <table class=\"table\">\n" +
                ...
                if(zhangPaperId != 0 && luPaperId != 0 && zhangPaperId == luPaperId && !(roleZhang.equals(roleLu)) ){
                    Statement stmt=con.createStatement();  
                    ResultSet rs=stmt.executeQuery("select * from conference_papers where paper_id="+zhangPaperId);
                    ...
                    while(rs.next()){
                        ...
                        String sAuthor = "-";
                        while(rs1.next()){
                            if(rs1.getString(2).equals("Zhang") || rs1.getString(2).equals("Lu")){
